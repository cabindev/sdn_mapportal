...
import { Eye, EyeOff, Filter, BarChart3 } from "lucide-react";
...
export default function RightSidebar({
  ...
  const [activePanel, setActivePanel] = useState<'filter' | 'stats' | null>(null);

  // นับจำนวนเอกสารในแต่ละหมวดหมู่
  const getCategoryCount = (categoryId: number) => {
    return documents.filter(doc => doc.categoryId === categoryId).length;
  };

  // จำนวนเอกสารทั้งหมดที่แสดงอยู่
  const visibleDocumentsCount = documents.filter(doc => 
    selectedCategories.length === 0 || selectedCategories.includes(doc.categoryId)
  ).length;

  // ฟังก์ชันสำหรับเปิด/ปิดการแสดงหมวดหมู่
  ...
  // ฟังก์ชันสำหรับเปิด/ปิดแผง
  const togglePanel = (panel: 'filter' | 'stats') => {
  ...
  return (
    <>
          ...
          <div className="border-b border-gray-200 p-4 bg-gray-50/50">
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs text-gray-600 bg-gray-100 px-2 py-1 rounded-full font-light">
                {visibleDocumentsCount}/{documents.length} รายการ
              </div>
            </div>
          </div>
      ...
      {/* Stats Panel */}
      {activePanel === 'stats' && (
        <div className="absolute top-44 right-20 z-[900] w-96 bg-white/95 rounded-xl shadow-lg border border-gray-200 overflow-hidden">
          {/* Header */}
          <div className="border-b border-gray-200 p-4 bg-gray-50/50">
            <div className="flex items-center justify-between">
              <h3 className="font-medium text-gray-900 text-sm flex items-center gap-2">
